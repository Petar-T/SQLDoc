...
#check if Target_Servers Table exists
	...
	[DiskSpace_Warning_Perc] [int] NULL,
	[LogSize_Warning_Gb] [int] NULL
) ON [PRIMARY] 
GO"
      Invoke-SqlCmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlCreateObj
      Write-host "Table $obj_Name created!" -ForegroundColor Yellow
...
GO"
 ...
 #check if BackupInfo Table exists
	...
	[logSize_mb] [decimal](18, 2) NULL,
	[rowSize_mb] [decimal](18, 2) NULL,
...
GO"
 ...
 #region tables : 'AGInfo_Table'
 #check if AGInfo Table exists
    $obj_Name = 'AG_Info'
    $sqlObj = "SELECT Count(*) as value_data FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '$obj_Name'"
    $res = Invoke-Sqlcmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlObj | select -expand value_data
    ...
    if ( $res -ne 1)
    {
    $sqlCreateObj = "CREATE TABLE [dbo].[Ag_Info](
	[ServerName] [nvarchar](50) NULL,
	[AGname] [nvarchar](50) NULL,
	[Listener_Name] [nvarchar](50) NULL,
	[primary_replica] [nvarchar](50) NULL,
	[primary_replica_Expected] [nvarchar](50) NULL,
	[Collection_Time] [datetime] NULL
) ON [PRIMARY]
GO"
      Invoke-SqlCmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlCreateObj
 ...
 #endregion tables : 'AGInfo_Table'

 #region tables : 'DiskInfo_Table'
 #check if DiskInfo Table exists
    $obj_Name = 'DiskInfo'
    $sqlObj = "SELECT Count(*) as value_data FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '$obj_Name'"
    $res = Invoke-Sqlcmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlObj | select -expand value_data
    ...
    if ( $res -ne 1)
    {
    $sqlCreateObj = "CREATE TABLE [dbo].[DiskInfo](
	[ServerName] [nvarchar](128) NULL,
	[Drive] [nvarchar](10) NULL,
	[Drive_Name] [nvarchar](50) NULL,
	[TotalSizeGBs] [decimal](12, 2) NULL,
	[AvailableGBs] [decimal](12, 2) NULL,
	[Collection_Date] [datetime] NULL
) ON [PRIMARY]
GO"
      Invoke-SqlCmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlCreateObj
 ...
 #endregion tables : 'DiskInfo_Table'

 #region tables : 'FileInfo_Table'
 #check if FileInfo Table exists
    $obj_Name = 'FileInfo'
    $sqlObj = "SELECT Count(*) as value_data FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '$obj_Name'"
    $res = Invoke-Sqlcmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlObj | select -expand value_data
    ...
    if ( $res -ne 1)
    {
    $sqlCreateObj = "CREATE TABLE [dbo].[FileInfo](
	[ServerName] [nvarchar](128) NULL,
	[Database_Name] [nvarchar](128) NULL,
	[Data_CurrentSizeMB] [decimal](18, 2) NULL,
	[recovery_model] [nvarchar](60) NULL,
	[total_vlf_count] [bigint] NULL,
	[Log_ActiveSizeMBs] [decimal](12, 2) NULL,
	[Log_CurrrentSizeMBs] [decimal](12, 2) NULL,
...
GO"
 ...
 #endregion tables : 'FileInfo_Table'
...
GO"
 ...
 #endregion tables : 'PerfMon_Table'
#endregion Tables
...
@logSize_mb decimal(18,2),		--P04
@rowSize_mb decimal(18,2),		--P05
@totalSize_mb decimal(18,2),	--P06
...
GO"
 ...
 #region Stored procedure : 'AGInfo_Manage'

#check if AGInfo_Manage proc exists
    $obj_Name = 'AGInfo_Manage'
    $sqlObj = "SELECT Count(*) as value_data FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME = '$obj_Name'"
    $res = Invoke-Sqlcmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlObj | select -expand value_data

    if ( $res -ne 1)
    {
    $sqlCreateObj = "CREATE Procedure [dbo].[AGInfo_Manage]
(
@ServerName nvarchar(50),		--P01
@AGname nvarchar(50),			--P02
@Listener_Name nvarchar(50),	--P03
@primary_replica nvarchar(50)	--P04

)					
AS
SET NOCOUNT ON;
IF EXISTS(select * from [dbo].[AG_Info] where [AGName]=@AGname and [ServerName]=@ServerName)
	UPDATE [dbo].[AG_Info]
	SET [Listener_Name] = @Listener_Name,
    [primary_replica] = @primary_replica,
    [Collection_Time] = GetDate() 
 WHERE ([AGName]=@AGname and [ServerName]=@ServerName)
 ELSE
 INSERT INTO [dbo].[AG_Info]
           ([ServerName]
           ,[AGname]
           ,[Listener_Name]
           ,[primary_replica]
...
GO"
 ...
 #endregion Stored procedure : 'AGInfo_Manage'

 #region Stored procedure : 'DiskInfo_Manage'

#check if DiskInfo_Manage proc exists
    $obj_Name = 'DiskInfo_Manage'
    $sqlObj = "SELECT Count(*) as value_data FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME = '$obj_Name'"
    $res = Invoke-Sqlcmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlObj | select -expand value_data

    if ( $res -ne 1)
    {
    $sqlCreateObj = "CREATE Procedure [dbo].[DiskInfo_Manage]
(
@ServerName nvarchar(128),		--P01
@Drive nvarchar(10),			--P02
@Drive_Name  nvarchar(50),		--P03
@TotalSizeGBs decimal(12,2),	--P04
@AvailableGBs decimal(12,2),	--P05
@Collection_Date datetime 		--P06
)					
AS
SET NOCOUNT ON;
IF EXISTS(select * from [dbo].[DiskInfo] where [Drive]=@Drive and [ServerName]=@ServerName)
	UPDATE [dbo].[DiskInfo]
SET [Drive_Name]=@Drive_Name,
	[TotalSizeGBs]=@TotalSizeGBs,
	[AvailableGBs]=@AvailableGBs,
	[Collection_Date]=@Collection_Date
 WHERE ([Drive]=@Drive and [ServerName]=@ServerName)
 ELSE
...
GO"
 ...
 #endregion Stored procedure : 'DiskInfo_Manage'

 #region Stored procedure : 'FileInfo_Manage'

#check if FileInfo_Manage proc exists
    $obj_Name = 'FileInfo_Manage'
    $sqlObj = "SELECT Count(*) as value_data FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME = '$obj_Name'"
    $res = Invoke-Sqlcmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlObj | select -expand value_data

    if ( $res -ne 1)
    {
    $sqlCreateObj = "CREATE Procedure [dbo].[FileInfo_Manage]
(
@ServerName nvarchar(128),			--P01
@Database_Name nvarchar(128),		--P02
@Data_CurrentSizeMB  decimal(18,2),	--P03
@recovery_model nvarchar(60),		--P04
@total_vlf_count bigint,			--P05
@Log_ActiveSizeMBs decimal(12,2),	--P06
@Log_CurrrentSizeMBs decimal(12,2),	--P07
@Collection_Date datetime 			--P08
)	

AS
SET NOCOUNT ON;
IF EXISTS(select * from [dbo].[FileInfo] where [Database_Name]=@Database_Name and [ServerName]=@ServerName)
	UPDATE [dbo].[FileInfo]
SET [Data_CurrentSizeMB]=@Data_CurrentSizeMB,
	[recovery_model]=@recovery_model,
	[total_vlf_count]=@total_vlf_count,
	[Log_ActiveSizeMBs]=@Log_ActiveSizeMBs,
	[Log_CurrrentSizeMBs]=@Log_CurrrentSizeMBs,
	[Collection_Date]=@Collection_Date
 WHERE ([Database_Name]=@Database_Name and [ServerName]=@ServerName)
 ELSE
...
GO"
 ...
 #endregion Stored procedure : 'FileInfo_Manage'
...
#check if AGInfo_Manage proc exists
    $obj_Name = 'AG_View'
    $sqlObj = "SELECT Count(*) as value_data FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_NAME = '$obj_Name'"
    $res = Invoke-Sqlcmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlObj | select -expand value_data

    if ( $res -ne 1)
    {
    $sqlCreateObj = "Create view AG_View 
  as
  SELECT 
    ServerName, 
    CASE 
        WHEN COUNT(CASE WHEN (ServerName=primary_replica_expected ) and (primary_replica!=primary_replica_Expected) THEN 1 END) > 0 THEN 'FALSE' 
        ELSE 'TRUE' 
    END AS [AG_OK]
    FROM Ag_Info
    GROUP BY ServerName
GO"
 ...
 #endregion View : 'AG_View'

 #region View : 'Job_View'

#check if Job_View exists
    $obj_Name = 'Job_View'
    $sqlObj = "SELECT Count(*) as value_data FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_NAME = '$obj_Name'"
    $res = Invoke-Sqlcmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlObj | select -expand value_data
    ...
    if ( $res -ne 1)
    {
    $sqlCreateObj = "Create   View [dbo].[Job_View] as
select srv.Servername,srv.[agentsvc_status] as SQLAgent_Status,
CASE WHEN EXISTS (select Servername
  from [JobInfo] ji
  where (ji.Is_enabled  =  1 and ji.Schedule_enabled=1 and ji.JobLastOutcome='Failed')
  and ji.ServerName = srv.ServerName)
       THEN 'False' 
       ELSE 'True'
  END AS JOBS_OK   
FROM JobInfo as srv
Group by srv.Servername,srv.[agentsvc_status]
GO"
      Invoke-SqlCmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlCreateObj
      Write-host "Procedure $obj_Name created!" -ForegroundColor Yellow
    } 

 #endregion View : 'Job_View'

 #region View : 'DiskFile_View'

#check if DiskFile_View exists
    $obj_Name = 'DiskFile_View'
    $sqlObj = "SELECT Count(*) as value_data FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_NAME = '$obj_Name'"
    $res = Invoke-Sqlcmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlObj | select -expand value_data

    if ( $res -ne 1)
    {
    $sqlCreateObj = "create view [dbo].[DiskFile_View]
AS
SELECT s.ServerName, 
IIF(Dinfo.ServerName IS NULL, 'TRUE', 'FALSE') AS Disk_OK, 
IIF(Finfo.ServerName IS NULL, 'TRUE', 'FALSE') AS LogMaxSize_OK
FROM  [dbo].[Target_Servers] s 
	LEFT OUTER JOIN
    (SELECT di.ServerName
         FROM  [dbo].[DiskInfo] di 
			INNER JOIN dbo.Target_Servers srv ON di.ServerName = srv.ServerName
         --WHERE AvailableGBs < IIF(srv.[DiskSpace_Warning_Gb] IS NULL, 20, srv.[DiskSpace_Warning_Gb])) Dinfo ON Dinfo.ServerName = s.ServerName 
		   WHERE (100*AvailableGBs/TotalSizeGBs) < IIF(srv.[DiskSpace_Warning_Perc] IS NULL, 10, srv.[DiskSpace_Warning_Perc])) Dinfo ON Dinfo.ServerName = s.ServerName 
	LEFT OUTER JOIN
    (SELECT DISTINCT fi.ServerName
         FROM  [dbo].[FileInfo] fi 
		 INNER JOIN [dbo].[Target_Servers] srv ON srv.ServerName = fi.ServerName
         WHERE fi.Log_CurrrentSizeMBs > IIF(srv.[LogSize_Warning_Gb] IS NULL, 10, srv.[LogSize_Warning_Gb]) * 1024) FInfo ON FInfo.ServerName = s.ServerName
GO"
      Invoke-SqlCmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $sqlCreateObj
      Write-host "Procedure $obj_Name created!" -ForegroundColor Yellow
    } 

 #endregion View : 'DiskFile_View'
...
SELECT 
    ...
    CAST(SUM(CASE WHEN type_desc = 'LOG' THEN size END) * 8. / 1024 AS DECIMAL(18,2)) as logSize_mb , 
    CAST(SUM(CASE WHEN type_desc = 'ROWS' THEN size END) * 8. / 1024 AS DECIMAL(18,2)) as  rowSize_mb , 
    CAST(SUM(size) * 8. / 1024 AS DECIMAL(18,2)) as totalSize_mb 
...
$Query="SELECT @@ServerName as ServerName,
	...
	[counter_name], 
	case cntr_type 
	when 65792 then 'Absolute'   
	when 65536 then 'Absolute' 
	when 272696320 then 'Cumulative' --Per Second counter and is Cumulative in Nature'
	when 272696576 then 'Cumulative' --Per Second counter and is Cumulative in Nature'
	when 537003264 then 'Bulk' --Bulk Counter. To get correct value, this value needs to be divided by Base Counter value' 
	when 1073874176 then 'Bulk' --Bulk Counter. To get correct value, this value needs to be divided by Base Counter value'
	when 1073939712 then 'Raw' --'Raw base'
	end as counter_type,
 	[instance_name],
	[cntr_value]
...
<#
    ...
    $InfoQry="SELECT srv.[ServerName], srv.[Environment_Type],ag_view.AG_OK,Job_view.SQLAgent_Status, Job_view.Jobs_OK,df.Disk_OK, df.LogMaxSize_OK,
	...
	left outer join ag_view on srv.ServerName=ag_view.ServerName
	left outer join job_view on srv.ServerName=job_view.ServerName
    left outer join DiskFile_view df on srv.ServerName = df.ServerName
	where srv.[is_Enabled]=1
	--order by srv.[ServerName]
	order by srv.[Environment_Type],srv.[ServerName]"
#>

    $InfoQry="SELECT srv.[ServerName], srv.[Environment_Type],
'<p class=""'+ IIF(ag_view.AG_OK='TRUE' , 'is_ok', 'is_not_ok') +'"">' + ag_view.AG_OK + '</p>' as AG_OK,
'<p class=""'+ IIF(Job_view.SQLAgent_Status='Running' , 'is_ok', 'is_not_ok') +'"">' + Job_view.SQLAgent_Status + '</p>' as SQLAgent_Status,
'<p class=""'+ IIF(Job_view.Jobs_OK='TRUE' , 'is_ok', 'is_not_ok') +'"">' + Job_view.Jobs_OK + '</p>' as Jobs_OK,
'<p class=""'+ IIF(df.Disk_OK='TRUE' , 'is_ok', 'is_not_ok') +'"">' + df.Disk_OK + '</p>' as Disk_OK,
'<p class=""'+ IIF(df.LogMaxSize_OK='TRUE' , 'is_ok', 'is_not_ok') +'"">' + df.LogMaxSize_OK + '</p>' as LogMaxSize_OK,
	-- '<a href=''.\' + [ServerName] + '.htm'' target=''_blank''>' + [ServerName] + '</a>' as Link, 	
	'<a href=''.\' + srv.[ServerName] + '\SQLDoc_Server.htm''><button> Server Info </button></a>' as ServerLink,
	'<a href=''.\' + srv.[ServerName] + '\SQLDoc-AssessmentData.htm''><button> Best Practices : ' + Cast(aa.cnt as varchar(4))  + '</button></a>' as AssessmentLink,
	'<a href=''.\' + srv.[ServerName] + '\SQLDoc_missing_index.htm''><button> Missing Indexes : ' + Cast(mi.cnt as varchar(4))  + '</button></a>' as MissingIndexLink,
    ...
    #$HTML = $ReportData | Select ServerName, Environment_Type, ServerLink, AssessmentLink,  MissingIndexLink, JobInfoLink, BackupInfoLink | ConvertTo-Html -CSSUri mi_style.css -Title "index page"  -PreContent "$($ReportHeader)" -PostContent "$($ReportFooter)"  
       ...
       $HTML = $ReportData | Select ServerName, Environment_Type, AG_OK, SQLAgent_Status, JOBS_OK, Disk_OK, LogMaxSize_OK, ServerLink, AssessmentLink,  MissingIndexLink, JobInfoLink, BackupInfoLink | ConvertTo-Html -CSSUri mi_style.css -Title "index page"  -PreContent "$($ReportHeader)" -PostContent "$($ReportFooter)"  
...
param(
 ...
 function SQLDoc-AGStatus-Collect
{
param(
 [Parameter(Mandatory = $true)]
        [String]
...
$Query="SELECT @@Servername as ServerName, Groups.[Name] AS AGname, Listeners.dns_name as Listener_Name, States.primary_replica
FROM sys.dm_hadr_availability_group_states States
INNER JOIN master.sys.availability_groups Groups ON States.group_id = Groups.group_id
Inner join master.sys.availability_group_listeners Listeners on states.group_id=Listeners.group_id 
order by AGname" 
    ...
    foreach($row in $srvrs)
    {
        ...
        try
        {
            $mi= Invoke-Sqlcmd -Query $Query -ServerInstance  $srv -Database 'master' -ErrorAction Stop
            Write-Host "[$srv] connected.. " -ForegroundColor Green
        ...
        {  
            $P_01=    $row.Item('ServerName')
            $P_02=    $row.Item('AGname')
            $P_03=    $row.Item('Listener_Name')
            $P_04=    $row.Item('primary_replica')
            #$P_20=    $row.Item('Collection_Time')


            try
            {
                $cmd.CommandText = "EXEC dbo.AGInfo_Manage '$P_01','$P_02','$P_03','$P_04'"
                $cmd.ExecuteNonQuery() | Out-Null
                $rowCount=$rowCount+1
            }
            catch [Exception] 
            {
                Write-Warning $_.Exception.Message
                $cmd.CommandText = "INSERT INTO Collection_Errors (ServerName,Collection_Type, Error_Message) VALUES ('$Srv','AG collection [1]','$_.Exception.Message')"
                $cmd.ExecuteNonQuery() | Out-Null                  
            }
        }
        ...
        catch [Exception]
        {
                Write-Warning "Problem connecting $srv"
                Write-Warning  $cmd.CommandText
                $cmd.CommandText = "INSERT INTO Collection_Errors (ServerName,Collection_Type, Error_Message) VALUES ('$Srv','AG Collection [2]','$_.Exception.Message')"
                $cmd.ExecuteNonQuery() | Out-Null
        }
    ...
    SQLDoc-Common-MakeLogEntry -DataWarehouseServer $DataWarehouseServer -DataWarehouseDatabase $DataWarehouseDatabase -Message 'AG Info collected!'
}


function SQLDoc-FileInfo-Collect
{
param(
 [Parameter(Mandatory = $true)]
        [String]
...
if ($DataWarehouseDatabase -eq '')
    ...
    $DiskQuery="SELECT DISTINCT
			    @@serverName as ServerName , 
			    s.volume_mount_point [Drive],
			    s.logical_volume_name [Drive_Name],
			    CAST(s.total_bytes / (1024 * 1024 * 1024) as decimal(12,2)) [TotalSizeGBs],
			    CAST(s.available_bytes / (1024 * 1024 * 1024) as decimal(12,2)) [AvailableGBs],
                Getdate() as [Collection_Date]
		        FROM sys.master_files f
			    CROSS APPLY sys.dm_os_volume_stats(f.database_id, f.[file_id]) s"

    $FileQuery= "select	@@SERVERNAME as ServerName,
			    dbs.name as Database_Name, 
				dat.Data_CurrentSizeMB,
				logs.recovery_model,
				logs.total_vlf_count,
				CAST(logs.active_log_size_mb  as decimal(12,2)) [Log_ActiveSizeMBs],
				CAST(logs.total_log_size_mb  as decimal(12,2)) [Log_CurrrentSizeMBs],
                Getdate() as [Collection_Date]
                    from sys.databases dbs 
                    cross apply sys.dm_db_log_stats (dbs.database_id) logs
	                    inner join (SELECT DB_NAME(database_id) AS database_name, 
					    Sum(mf.size/128.0) AS Data_CurrentSizeMB
					    FROM sys.master_files mf WHERE type <> 1
					    group by DB_NAME(database_id)) dat on dat.database_name=dbs.name" 
    ...
    foreach($row in $srvrs)
    {
        ...
        try
        {
            $mi= Invoke-Sqlcmd -Query $DiskQuery -ServerInstance  $srv -Database 'master' -ErrorAction Stop
            Write-Host "[$srv] connected.. " -ForegroundColor Green
        ...
        foreach ($row in $mi)
        {
            $P_01=    $row.Item('ServerName')
            $P_02=    $row.Item('Drive')
            $P_03=    $row.Item('Drive_Name')
            $P_04=    $row.Item('TotalSizeGBs')
            $P_05=    $row.Item('AvailableGBs')
            $P_06=    $row.Item('Collection_Date')
    
            try
            {
                $cmd.CommandText = "EXEC dbo.DiskInfo_Manage '$P_01','$P_02','$P_03',$P_04,$P_05,'$P_06' "
                $cmd.ExecuteNonQuery() | Out-Null
                $rowCount=$rowCount+1
            }
            catch [Exception] 
            {
                Write-Warning $_.Exception.Message
                $cmd.CommandText = "INSERT INTO Collection_Errors (ServerName,Collection_Type, Error_Message) VALUES ('$Srv','File&Disk info [1]','$_.Exception.Message')"
                $cmd.ExecuteNonQuery() | Out-Null                  
            }
        }
        ...
        {
                Write-Warning "Problem connecting $srv"
                $cmd.CommandText = "INSERT INTO Collection_Errors (ServerName,Collection_Type, Error_Message) VALUES ('$Srv','File&Disk Collection [2]','$_.Exception.Message')"
                $cmd.ExecuteNonQuery() | Out-Null
        }
        ...
        try
        {
            $mi= Invoke-Sqlcmd -Query $FileQuery -ServerInstance  $srv -Database 'master' -ErrorAction Stop
            Write-Host "[$srv] connected.. " -ForegroundColor Green
        ...
        {
            $P_01=    $row.Item('ServerName')
            $P_02=    $row.Item('Database_Name')
            $P_03=    $row.Item('Data_CurrentSizeMB')
            $P_04=    $row.Item('recovery_model')
            $P_05=    $row.Item('total_vlf_count')
            $P_06=    $row.Item('Log_ActiveSizeMBs')
            $P_07=    $row.Item('Log_CurrrentSizeMBs')
            $P_08=    $row.Item('Collection_Date')

                #fields are null on secondary server
                if ($row.IsNull('total_vlf_count') ) {    $P_05="Null"   }
                if ($row.IsNull('Log_ActiveSizeMBs') ) {    $P_06="Null"   }
                if ($row.IsNull('Log_CurrrentSizeMBs') ) {    $P_07="Null"   } 

        
            try
            {
                $cmd.CommandText = "EXEC dbo.FileInfo_Manage '$P_01','$P_02',$P_03,'$P_04',$P_05,$P_06,$P_07, '$P_08' "
                $cmd.ExecuteNonQuery() | Out-Null
                $rowCount=$rowCount+1
            }
            catch [Exception] 
    ...
    SQLDoc-Common-MakeLogEntry -DataWarehouseServer $DataWarehouseServer -DataWarehouseDatabase $DataWarehouseDatabase -Message 'File&Disk Info collected!'
}
...
#collect data
SQLDoc-Common-CheckCollectionDB -DataWarehouseServer $DataWarehouseServer -DataWarehouseDatabase $DataWarehouseDatabase
SQLDoc-AGStatus-Collect -DataWarehouseServer $DataWarehouseServer -DataWarehouseDatabase  $DataWarehouseDatabase
...
SQLDoc-BadLogins-Collect  -DataWarehouseServer  $DataWarehouseServer -DataWarehouseDatabase  $DataWarehouseDatabase -PasswordListPath 'C:\SQL_Monitor\pwd_List.txt'
SQLDoc-FileInfo-Collect -DataWarehouseServer $DataWarehouseServer -DataWarehouseDatabase $DataWarehouseDatabase
}

function SQLDoc-Collect-And-Report-All
{
param(
 [Parameter(Mandatory = $true)]
        [String]
...
#generate report
SQLDoc-Report-All -DataWarehouseServer $DataWarehouseServer -DataWarehouseDatabase  $DataWarehouseDatabase  -ReportFolder 'C:\SQL_Monitor\Report'
}


function SQLDoc-ClearData
{
param(
 [Parameter(Mandatory = $true)]
        [String]
...
if ($DataWarehouseDatabase -eq '')
    ...
    FOR 
    Select Object_name(object_id) AS TableList from sys.objects where type = 'U' 
    and Object_name(object_id) NOT IN ('Target_Servers')

    OPEN cur
    FETCH NEXT FROM cur INTO @TableList
    ...
    SET @statement = 'DELETE FROM ' + @TableList;
    print @statement
    ...
    Try
    {
        #$srvrs= Invoke-SqlCmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $RemoveDataQuery
        Invoke-SqlCmd -ServerInstance $DataWarehouseServer -Database $DataWarehouseDatabase -Query $RemoveDataQuery -ErrorAction Stop
        Write-Host "removing data.. " -ForegroundColor Green
    ...
    Catch 
    {
                ...
                $scon.Open()
                    ...
                    #$cmd.CommandText = "INSERT INTO Collection_Errors (ServerName,Collection_Type, Error_Message) VALUES ('$DataWarehouseServer','Data Removal [1]','$_.Exception.Message')"
                    $cmd.CommandText = "INSERT INTO Collection_Errors (ServerName,Collection_Type, Error_Message) VALUES ('$DataWarehouseServer','Data Removal [1]',N'$($_.Exception.Message -replace "'", "''")')"
                    $cmd.ExecuteNonQuery() | Out-Null   
    ...
    SQLDoc-Common-MakeLogEntry -DataWarehouseServer $DataWarehouseServer -DataWarehouseDatabase $DataWarehouseDatabase -Message 'Data removed'
}
 ...
 Export-ModuleMember -Function SQLDoc-AGStatus-Collect
 Export-ModuleMember -Function SQLDoc-Collect-All
 Export-ModuleMember -Function SQLDoc-Collect-And-Report-All
 Export-ModuleMember -Function SQLDoc-FileInfo-Collect 

 Export-ModuleMember -Function SQLDoc-ClearData

 <#

 Example usage:

 SQLDoc-FileInfo-Collect -DataWarehouseServer MyServ -DataWarehouseDatabase SQL_Datawarehouse
 SQLDoc-Report-All -DataWarehouseServer MyServ -DataWarehouseDatabase  SQL_Datawarehouse  -ReportFolder 'C:\SQL_Monitor\Report'
 SQLDoc-AGStatus-Collect -DataWarehouseServer MyServ -DataWarehouseDatabase  SQL_Datawarehouse
 SQLDoc-Common-CheckCollectionDB -DataWarehouseServer MyServ -DataWarehouseDatabase SQL_Datawarehouse
 SQLDoc-MissingIndexes-Collect -DataWarehouseServer MyServ -DataWarehouseDatabase SQL_Datawarehouse

 SQLDoc-Jobs-Collect -DataWarehouseServer  MyServ -DataWarehouseDatabase SQL_Datawarehouse
 SQLDoc-Backups-Collect -DataWarehouseServer  MyServ -DataWarehouseDatabase SQL_Datawarehouse
 SQLDoc-Main-CollectGeneralInfo -DataWarehouseServer  MyServ -DataWarehouseDatabase  SQL_Datawarehousee 
 SQLDoc-BadLogins-Collect  -DataWarehouseServer  MyServ -DataWarehouseDatabase  SQL_Datawarehouse -PasswordListPath 'C:\SQL_Monitor\pwd_List.txt'
 
 SQLDoc-Report-All -DataWarehouseServer MyServ -DataWarehouseDatabase  SQL_Datawarehouse  -ReportFolder 'C:\SQL_Monitor\Report'
 SQLDoc-Report-GeneralInfo -DataWarehouseServer MyServ -DataWarehouseDatabase SQL_Datawarehouse -ReportFolder "C:\Users\petartr\Desktop\POwershell Module PetarT\Report"  -CreateSubReports
 SQLDoc-Main-CollectGeneralInfo -DataWarehouseServer MyServ -DataWarehouseDatabase SQL_Datawarehouse

 SQLDoc-Jobs-Collect -DataWarehouseServer MyServ -DataWarehouseDatabase SQL_Datawarehouse
 SQLDoc-Backups-Collect -DataWarehouseServer MyServ -DataWarehouseDatabase SQL_Datawarehouse
 SQLDoc-BadLogins-Collect -DataWarehouseServer MyServ -DataWarehouseDatabase SQL_Datawarehouse -PasswordListPath 'C:\tempExport\pwdList.txt'
#>